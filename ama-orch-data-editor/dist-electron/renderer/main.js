...
resize();
function addObject(name, position, color = '#f07c69') {
...
function selectObject(mesh) {
    if (selected)
        selected.material.color.set(selected.userData.baseColor);
    selected = mesh;
    mesh.material.color.set('#62b5d8');
...
void (0, sql_js_1.default)({ locateFile: (file) => new URL(`/node_modules/sql.js/dist/${file}`, window.location.origin).href }).then((SQL) => {
